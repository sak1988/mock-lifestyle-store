...
/**
 * Home page that reads products + site config from Supabase (read-only via ANON key).
 * - Needs these Vercel env vars:
 *   NEXT_PUBLIC_SUPABASE_URL, NEXT_PUBLIC_SUPABASE_ANON_KEY
 * - Arm is still forced by env: NEXT_PUBLIC_FRAME_ARM = 'persona' | 'feature'
 */

const FORCED_ARM =
  process.env.NEXT_PUBLIC_FRAME_ARM === 'feature' ? 'feature' : 'persona';
...
function StarRating({ value = 0 }) {
  ...
  return (
    <div className="flex items-center gap-0.5" aria-label={`Rating ${value}`}>
      ...
      {half && <span>☆</span>}
      {Array.from({ length: Math.max(empties, 0) }).map((_, i) => (
        <span key={'e' + i}>☆</span>
      ))}
...
const Badge = ({ children }) => (
  <span className="px-2 py-1 rounded-full bg-zinc-100 text-zinc-700 text-xs border">
    {children}
  </span>
);
const Button = ({ children, onClick, className = '' }) => (
  <button
    onClick={onClick}
    className={`px-3 py-2 rounded-xl border hover:bg-zinc-50 ${className}`}
  >
    {children}
  </button>
);
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 grid md:grid-cols-2 gap-6 items-center">
        <div>
          <h1 className="text-2xl md:text-3xl font-semibold mb-2">
            Fresh picks for everyday living
          </h1>
          {arm === 'persona' ? (
            <p className="text-zinc-700">
              Find gear that fits <span className="font-medium">who you are</span>—from lazy-morning
              walkers to deep-work sprinters.
            </p>
          ) : (
            <p className="text-zinc-700">
              Shop by <span className="font-medium">features</span> that matter—cushioning, battery
              life, water resistance, and more.
            </p>
          )}
        </div>
...
function ProductCard({ p, arm, onAdd }) {
  const lead = arm === 'persona' ? p.persona_text : p.feature_text;
  return (
...
function Grid({ products, arm, onAdd }) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {products.map((p) => (
        <ProductCard key={p.id} p={p} arm={arm} onAdd={onAdd} />
      ))}
    </div>
  );
}
...
function CartDrawer({ open, items, onClose }) {
  ...
  return (
    ...
    >
      ...
      />
      <div
        className={`absolute right-0 top-0 h-full w-full max-w-md bg-white border-l shadow-xl transition-transform ${
...
export default function Page() {
    ...
    async function load() {
      try {
        const [cfgRes, prodRes] = await Promise.all([
          fetch(`${URL}/rest/v1/site_config?id=eq.1&select=*`, {
            headers: { apikey: ANON, Authorization: `Bearer ${ANON}` },
          }),
          fetch(`${URL}/rest/v1/products?select=*&order=created_at.desc`, {
            headers: { apikey: ANON, Authorization: `Bearer ${ANON}` },
          }),
        ]);
  ...
  const filtered = useMemo(() => {
    ...
    let items = (catalog || []).filter((p) =>
      [p.name, p.brand, ...(p.tags || []), p.persona_text, p.feature_text]
        .filter(Boolean)
          ...
          <div className="space-y-6">
            <div>
              ...
              <ul className="space-y-2 text-sm">
                {['Footwear', 'Apparel', 'Accessories', 'Electronics', 'Sleep & Recovery'].map(
                  (d) => (
